...
Each row in the dataset represents one instance of delay, specifying its route, date and time, location, classification of bus incident, and its delay duration.
...
A snippet of the dataset is show in @tbl-ttc_head.
...
Kennedy Station stands out as the site with the highest number of delays, suggesting that either infrastructural limitations or a high volume of passengers are contributing to these frequent interruptions.

The EDA analysis was very informative in understanding the columns of interest for this project, which will be used to create the logistic regression model to predict the expected delay. The delay output will be categorized into "Short", "Medium" or "Long". Cross-validation and randomized grid search were applied for hyperparameter tuning to enhance model performance.
